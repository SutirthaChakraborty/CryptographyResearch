...
def mpz(n):
  '''A dummy function to ensure compatibility with those that do not have gmpy. Returns n.'''
  return n
...
RECORD = 187072209578355573530071658587684226515959365500927
fo = open("FactorDriver_output.txt", "w")
...
def sure_factors(n, u, curve_params, tdb):
...
def get_points(p1, n):
 ...
 Returns the points.'''
    ...
    for j in range(ncurves):
        ...
        x_0 = (b * x_0) % n
        y_0 = (b ** 2) % n
        print("Curve ", j, " P is (", x_0, ", ", y_0, ")")
        print("Curve ", j, "A is ", a, " and C is ", b)
        fo.write("\n Curve "+ str(j)+ " P is ("+ str(x_0)+ ", "+ str(y_0)+ ")\n")
        fo.write("\n Curve "+ str(j)+ "A is ("+ str(a)+ " and C is "+ str(b)+")\n")
        while a % 3:
            a += n
...
def ecm(n, tdb, nc):  # DOCUMENTATION
    '''Input:
 ...
 2. If ra is set to False and veb is set to True, then results are reproducible. If ra is set to True, then one number may be done in parallel on disconnected machines (at only a small loss of efficiency, which is less if pr is set correctly).'''
    ...
    for i in range(len(prime_probs)):
        ...
        p_fail = max(0, (1 - p_success * math.log(math.log(k)))) ** (k)
    ...
    while n != 1:
        ...
        for factor in sure_factors(n, u, list(range(sigma, sigma + nc)), tdb):
            yield factor
            n = n // factor

        for i in range(len(prime_probs)):
            p_success = rho_ev((i - 2.65) / math.log(u), t)
            p_fail = max(0, (1 - p_success * math.log(math.log(u)))) ** nc
            prime_probs[i] = p_fail * prime_probs[i] / (p_fail * prime_probs[i] + 1 - prime_probs[i])
...
def factors(n, nc):
 ...
 2. If ra is set to False and veb is set to True, then results are reproducible. If ra is set to True, then one number may be done in parallel on disconnected machines (at only a small loss of efficiency, which is less if pr is set correctly).'''
    ...
    for factor in ecm(n, trial_division_bound, nc):
        yield factor
...
def getECMFactors(n, numOfCurvePerGo):
     for factor in factors(n, numOfCurvePerGo):
         print("\n Factor of "+str(n)+ " is ", factor)
         fo.write("\n Factor of "+str(n)+ " is "+ str(factor))
